...
export function EditProfileModal({ isOpen, onClose, onSave, initialData }: EditProfileModalProps) {
  const [formData, setFormData] = useState<UserProfileData>({
    ...
    avatar: initialData?.avatar || 'https://api.dicebear.com/7.x/avataaars/svg?seed=You',
  });
  ...
  const validateForm = (): boolean => {
    ...
    if (formData.website && !isValidUrl(formData.website) && !formData.website.startsWith('http')) {
      newErrors.website = 'Nieprawidłowy adres URL (użyj formatu https://example.com)';
    }
  ...
  const handleSubmit = async (e: FormEvent) => {
    ...
    try {
      ...
      setErrors({ 
        name: error instanceof Error ? error.message : 'Nie udało się zapisać profilu. Spróbuj ponownie.' 
      });
    } finally {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 backdrop-blur-sm p-4">
      <div 
        className="bg-[#1e1e2e] rounded-lg border border-gray-800 w-full max-w-2xl max-h-[90vh] overflow-y-auto shadow-[0_0_20px_rgba(0,238,255,0.2)]"
        style={{
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-6">
          ...
          <div>
            ...
            <input
              ...
              className={`w-full px-4 py-3 rounded-lg border transition-all ${
                errors.name ? 'border-red-500' : 'border-gray-700 focus:border-[#00eeff]'
              }`}
              style={{
          ...
          <div>
            ...
            <textarea
              ...
              className={`w-full px-4 py-3 rounded-lg border transition-all resize-none ${
                errors.bio ? 'border-red-500' : 'border-gray-700 focus:border-[#00eeff]'
              }`}
              style={{
                backgroundColor: 'var(--bg)',
                color: 'var(--text-primary)'
          ...
          <div>
            ...
            <input
              ...
              className={`w-full px-4 py-3 rounded-lg border transition-all ${
                errors.website ? 'border-red-500' : 'border-gray-700 focus:border-[#00eeff]'
              }`}
              style={{
                backgroundColor: 'var(--bg)',
                color: 'var(--text-primary)'
            ...
            <p className="mt-1 text-xs" style={{ color: 'var(--text-secondary)' }}>
              URL zostanie automatycznie sformatowany z https:// jeśli nie zostanie podany
            </p>
          </div>
